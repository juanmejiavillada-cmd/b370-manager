#!/usr/bin/env python3
"""Corrige el ID correcto de la página Colombia (3293, no 3294) y despliega todo."""
...
# ── 1. Actualizar b370-landing-styles.php con ID correcto ─────────────────
LANDING_STYLES = r"""<?php
/**
...
print("1. Actualizando b370-landing-styles.php con ID 3293...")
with sftp.open(f"{MU}/b370-landing-styles.php", "wb") as f:
    f.write(LANDING_STYLES.encode("utf-8"))
print("   OK")

# ── 2. Actualizar el override con ID correcto ──────────────────────────────
MU_OVERRIDE = r"""<?php
/**
 * B370 Colombia — Override de contenido
 * ID correcto: 3293
 */
if (!defined('ABSPATH')) exit;
...
print("2. Actualizando b370-colombia-override.php con ID 3293...")
with sftp.open(f"{MU}/b370-colombia-override.php", "wb") as f:
    f.write(MU_OVERRIDE.encode("utf-8"))
print("   OK")

sftp.close()

# ── 3. Validar PHP ─────────────────────────────────────────────────────────
print("3. Validando PHP...")
out = run(f"php -l {MU}/b370-colombia-override.php && php -l {MU}/b370-landing-styles.php")
print(f"   {out[:200]}")

# ── 4. Verificar con curl que ahora aparece el CSS ─────────────────────────
print("4. Verificando CSS en HTML servido...")
out2 = run(
    "curl -s 'https://b370sports.com/colombia-mundial-2026/' --max-time 20 "
    "| grep -c 'b370-colombia-css'"
)
print(f"   b370-colombia-css en HTML: {out2} instancias")

out3 = run(
    "curl -s 'https://b370sports.com/colombia-mundial-2026/' --max-time 20 "
    "| grep -o 'b370-col-[a-z-]*' | sort -u | head -10"
)
print(f"   Clases b370-col: {out3 or '(ninguna — caché aun)'}")

c.close()
print("\nTodo corregido con ID 3293. Haz Ctrl+Shift+R en el browser.")
