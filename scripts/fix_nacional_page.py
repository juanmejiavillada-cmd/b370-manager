...
# ─── 1. CSS VA EN EL MU-PLUGIN (inyectado en <head> solo para page ID 3295) ───
css_nacional = """
 ...
 * Description: Inyecta CSS de las landing pages (Nacional, Colombia) en el <head> solo para esas páginas.
 * Version: 1.0
 */
...
# ─── 2. HTML LIMPIO (sin style tags) para la página 3295 ───
html_clean = """<div class="b370-nac">
...
// Bypass kses sanitization so wp:html block is saved correctly
kses_remove_filters();

$html = file_get_contents('/tmp/nac_html.txt');
...
# Write local temp files
scripts_dir = os.path.dirname(__file__)
css_path  = os.path.join(scripts_dir, "nac_tmp.css")
php_path  = os.path.join(scripts_dir, "nac_tmp.php")
html_path = os.path.join(scripts_dir, "nac_tmp_html.txt")

with open(css_path,  "w", encoding="utf-8") as f: f.write(css_nacional)
with open(php_path,  "w", encoding="utf-8") as f: f.write(php_update)
with open(html_path, "w", encoding="utf-8") as f: f.write(html_clean)

mu_path = os.path.join(scripts_dir, "nac_tmp_mu.php")
with open(mu_path, "w", encoding="utf-8") as f: f.write(mu_plugin)
...
sftp.put(css_path,  f"{mu_dir}/b370-nacional.css")
sftp.put(mu_path,   f"{mu_dir}/b370-landing-styles.php")
sftp.put(html_path, "/tmp/nac_html.txt")
sftp.put(php_path,  "/tmp/nac_update.php")
sftp.close()

stdin, stdout, stderr = ssh.exec_command("php /tmp/nac_update.php 2>/dev/null")
result = stdout.read().decode().strip()
print("Page update:", result)

ssh.exec_command(f"cd {wp_path} && wp litespeed-purge all 2>/dev/null")
ssh.exec_command("rm /tmp/nac_html.txt /tmp/nac_update.php")
ssh.close()

for f in [css_path, php_path, html_path, mu_path]:
    os.remove(f)

print("Done — CSS in mu-plugin, HTML clean in page")
